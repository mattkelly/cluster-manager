...
// List lists all Plugins in the indexer.
func (s *pluginLister) List(selector labels.Selector) (ret []*v3.Plugin, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v3.Plugin))
	})
	return ret, err
}
...
// List lists all Plugins in the indexer for a given namespace.
func (s pluginNamespaceLister) List(selector labels.Selector) (ret []*v3.Plugin, err error) {
...
// Get retrieves the Plugin from the indexer for a given namespace and name.
func (s pluginNamespaceLister) Get(name string) (*v3.Plugin, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v3.Resource("plugin"), name)
	}
	return obj.(*v3.Plugin), nil
}

...
// List lists all Registries in the indexer.
func (s *registryLister) List(selector labels.Selector) (ret []*v3.Registry, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v3.Registry))
	})
	return ret, err
}
...
// List lists all Registries in the indexer for a given namespace.
func (s registryNamespaceLister) List(selector labels.Selector) (ret []*v3.Registry, err error) {
...
// Get retrieves the Registry from the indexer for a given namespace and name.
func (s registryNamespaceLister) Get(name string) (*v3.Registry, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v3.Resource("registry"), name)
	}
	return obj.(*v3.Registry), nil
}
